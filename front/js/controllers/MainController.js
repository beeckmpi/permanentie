...
var permanentie = angular.module( 'permanentie', ['ngRoute'] );
permanentie.factory('authInterceptor', function ($rootScope, $q, $window, $location) {
  return {
...
permanentie.config(function ($httpProvider) {
  $httpProvider.interceptors.push('authInterceptor');
});
permanentie.run(function($rootScope, $location, $window, $http){  
  $rootScope.go = function ( path ) {
    $location.path( path );
  };
});
permanentie.controller("MainController", ['$scope', '$rootScope', '$http', '$window', function($scope, $rootScope, $http, $window) {
...
permanentie.controller("LoginController", ['$scope', '$rootScope', '$http', '$window', function($scope, $rootScope, $http, $window){
...
permanentie.controller("ListController", ['$scope', '$rootScope', '$http', '$window', '$location', function($scope, $rootScope, $http, $window, $location){
  ...
  $http.get('/api/onteigeningen').success(function(data, status, headers, config){
    var onteigeningen = data.onteigeningen;
    angular.forEach(onteigeningen, function(onteigening){
      var d = new Date(onteigening.onteigening.created_at);
      var data = time_calc(d); 
      onteigening.created_at = data.datumTijd;
      
    });
    console.log(ObjectLength(onteigeningen));
    $rootScope.onteigeningen = onteigeningen;
  });
}]);
permanentie.controller("UserController", ['$scope', '$rootScope', '$http', '$window', function($scope, $rootScope, $http, $window){
  $http({url: '/api/users/'+$rootScope.user.id, method: 'GET'}).success(function(data, status, headers, config){
    $rootScope.signedIn = false;  
    $rootScope.LoggedOut = true;              
    $rootScope.user = data.user.google;    
...
permanentie.controller("AdminController", ['$scope', '$rootScope', '$http', '$window', function($scope, $rootScope, $http, $window){
